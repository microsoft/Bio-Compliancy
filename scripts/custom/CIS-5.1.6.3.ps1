...
$result = $true
if ($pol.AllowInvitesFrom -notin @('AdminsAndGuestInviters', 'None'))
{
    $result = $false
}

$script:exportResults += [PSCustomObject]@{
    ResourceName               = 'AADCustomLimitedGuestUserInvitations'
    ResourceInstanceName       = 'CIS-5.1.6.3'
    Id                         = 'Ensure guest user invitations are limited to the Guest Inviter role'
    GuestInvitationsAreLimited = $result
}
